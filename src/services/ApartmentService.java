...
public class ApartmentService {
	...
	public Reservation makeReservation(@Context HttpServletRequest request, Reservation reservation) {
		Date toDate = new Date(
				reservation.getStartDate().getTime() + reservation.getStayNights() * 60 * 60 * 24 * 1000);
		if (SearchService.apartmentFreeForDateSpan(ApartmentRepository.getInstance().read(reservation.getApartmentId()),
				reservation.getStartDate(), toDate)) {
			reservation.setTotalCost(SearchService.getCostFactorForDates(reservation.getStartDate(), toDate)
					* ApartmentRepository.getInstance().read(reservation.getApartmentId()).getNightStayPrice());
